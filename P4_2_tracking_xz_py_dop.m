...
% Doppler
dop_numen = 20; % no. of ensembles
dop_freq = 2.5; % MHz
ref_freq = 3; % MHz, have to be same as Trans.frequency
dop_M = 1; % order of high PRF Doppler
dop_N = 2; % doppler range gate start depth / doppler range gate size
dop_rangedepth = 64 *(c0/(ref_freq*1e6)); % [m] range gate size should be a multiple of 16
...
SFormat(1).numRays = n_ray;      % no. of Rays (1 for Flash transmit)
SFormat(1).FirstRayLoc = Trans.ElementPos(1,1:3);   % x,y,z
SFormat(1).rayDelta = 2*(-SFormat(1).theta)/(n_ray-1);  % spacing in radians(sector) or dist. between rays
...
SFormat(2).scanFormat = 'VAPX';
SFormat(2).theta = -img_angle;
SFormat(2).radius = (aperture/2)/tan(-SFormat(2).theta);
SFormat(2).numRays = 1;
SFormat(2).FirstRayLoc = [0,0,0];
SFormat(2).rayDelta = -SFormat(2).theta*2;
SFormat(2).startDepth = start_depth;
SFormat(2).endDepth = ceil(dop_rangegate(2)*1e-2/lambda); 
...
PData(2).pdeltaX = 0.5;
PData(2).pdeltaZ = 0.5;
PData(2).Size(1) = dop_rangedepth / (c0/(ref_freq*1e6)) /PData(2).pdeltaZ;
PData(2).Size(2) = ceil(dop_rangewidth*1e-2/lambda/PData(2).pdeltaX);
if mod(PData(2).Size(2),2) == 0
    PData(2).Size(2) = PData(2).Size(2) +1;
end
PData(2).Size(3) = 1;
PData(2).Origin = [-((PData(2).Size(2)-1)/2)*PData(2).pdeltaX,0,ceil(dop_rangegate(1)*1e-2/lambda)];

PData(3).sFormat = 1;
PData(3).pdeltaX = 1;
PData(3).pdeltaZ = 1;
PData(3).Size(1) = PData(1).Size(1);
PData(3).Size(2) = PData(1).Size(2);
PData(3).Size(3) = 1;
PData(3).Origin = PData(1).Origin;

Resource.RcvBuffer(1).datatype = 'int16';
Resource.RcvBuffer(1).rowsPerFrame = 4*n_ray*4096;
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = Nframe +1; 
...
Resource.InterBuffer(2).pagesPerFrame = dop_numen;
...
TW(1).Parameters = [30,14,2,1]; % 2.5 MHz% TW(1).Parameters = [36,17,2,1]; % 2.5 MHz

% Doppler
dop_twA = round(180 /(2*dop_freq));
if dop_twA <= 30
    dop_twB = round(0.67*dop_twA);
else
    dop_twB = round(0.33*dop_twA);
end
TW(2).type = 'parametric';
TW(2).Parameters = [dop_twA,dop_twB,6,1]; 

TX = repmat(struct('waveform', 1, ...
                   'Origin', [0.0,0.0,0.0], ...
                   'focus', ray_focus*1E-3/lambda, ...
                   'Steer', [0.0,0.0], ...
                   'Apod', ones(1,Trans.numelements), ...  % set TX.Apod for 64 elements
                   'Delay', zeros(1,Trans.numelements)), 1, n_ray+2);

% - Set event specific TX attributes.
beamWidth = 0.0; % approximate beam width in radians
Angles = SFormat(1).theta:SFormat(1).rayDelta:(SFormat(1).theta + (n_ray-1)*SFormat(1).rayDelta);
TXorgs = SFormat(1).radius*tan(Angles);
for n = 1:n_ray
...
% Listening TX event
TX(n_ray+1).Apod = zeros(1,Trans.numelements);

% Doppler TX event
TX(n_ray+2).waveform = 2;
TX(n_ray+2).focus = 0;
...
% TGC for listening
TGC(2).CntrlPts = ones(1,8)*500;
TGC(2).rangeMax = lstn_time*Resource.Parameters.speedOfSound/lambda;
TGC(2).Waveform = computeTGCWaveform(TGC(2));
% TGC for Doppler
TGC(3).CntrlPts = [ones(1,8)*900];
TGC(3).rangeMax = SFormat(2).endDepth;
TGC(3).Waveform = computeTGCWaveform(TGC(2));
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'callMediaFunc', 0),1,n_ray*Nframe+lstn_rcv+dop_numen);
                                       
% Set receive struct for Bmode
for i = 1:Nframe
    k = n_ray*(i-1);
    for j = 1:n_ray
        Receive(k+j).Apod(1:Trans.numelements) = 1.0;
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;  
    end
end
% Set receive struct for listening
k = n_ray*Nframe;
for i=1:lstn_rcv
    Receive(k+i).Apod(1:Trans.numelements) = 1;
    Receive(k+i).framenum = Nframe + 1;
    Receive(k+i).acqNum = i;
    Receive(k+i).TGC = 2;
    Receive(k+i).InputFilter = [0,0,0,0,0,1];
    Receive(k+i).startDepth = 0;
    Receive(k+i).endDepth = max_end_depth;
end
% Set receive struct for Doppler

k = n_ray*Nframe+lstn_rcv;
for j = 1:dop_numen
    Receive(k+j).Apod = ones(1,Trans.numelements);
    Receive(k+j).startDepth = 52;
    Receive(k+j).endDepth = Receive(k+j).startDepth+ 64;
    Receive(k+j).TGC = 3;
    Receive(k+j).InputFilter = [0,0,0,0,0,1];
    Receive(k+j).acqNum = lstn_rcv+j;
    Receive(k+j).framenum = Nframe + 1;
end
...
Recon(2) = struct('senscutoff', 0.6, ...
               ...
               'IntBufDest', [2,1], ...
               'ImgBufDest', [0,0], ...
               'RINums', n_ray+(1:dop_numen)');           

% Define ReconInfo structures.
ReconInfo = repmat(struct('mode', 0, ... 
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 0), 1, n_ray);
...
for j = 1:dop_numen
    ReconInfo(n_ray+j).mode = 3;
    ReconInfo(n_ray+j).txnum = n_ray+2;
    ReconInfo(n_ray+j).rcvnum = n_ray*Nframe+lstn_rcv+j;
    ReconInfo(n_ray+j).pagenum = j;
end
...
SeqControl(8).argument = round(1/(dop_prf*1e-06));
...
for i = 1:Nframe
    for j = 1:n_ray                     
        ...
        Event(n).rcv = n_ray*(i-1)+j;   
        Event(n).recon = 0;      % no reconstruction.
        Event(n).process = 0;    % no processing
        Event(n).seqControl = 2; % seqCntrl
        n = n+1;
    end
        Event(n-1).seqControl = [3,nsc];
            SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer % it should be here not preassigned in the sequence definition. Otherwise, script running time will be slow
        nsc = nsc+1;
    ...
    Event(n).info = 'save bmode post firing'; 
    Event(n).tx = 0;         % no transmit
    Event(n).rcv = 0;        % no rcv
    Event(n).recon = 0;      % reconstruction
    Event(n).process = 6;    % process
    Event(n).seqControl = 0;
    n = n+1;
end
...
Event(n).seqControl = nsc;
    ...
    SeqControl(nsc).argument = 2+(n_ray+5)*Nframe+6+lstn_rcv+5+dop_numen+1; %this event should be event called 'condition' number
    nsc = nsc+1;
n = n+1;
...
Event(n).info = 'Save Bmode frame before firing'; 
Event(n).tx = 0;         % no transmit
Event(n).rcv = 0;        % no rcv
Event(n).recon = 0;      % reconstruction
...
for i=1:lstn_rcv
    Event(n).info = 'listening';
    Event(n).tx = n_ray+1;   % use next TX structure.
    Event(n).rcv = n_ray*Nframe+i;   
    Event(n).recon = 0;      % no reconstruction.
    Event(n).process = 0;    % no processing
    Event(n).seqControl = 0; % seqCntrl
    n = n+1;
end   

% Insert doppler event here
for i = 1:dop_numen
    Event(n).info = 'Acquire Doppler'; 
    Event(n).tx = n_ray+2;         % no transmit
    Event(n).rcv = n_ray*Nframe+lstn_rcv+i;        % no rcv
    Event(n).recon = 0;      % reconstruction
    Event(n).process = 0;    % process
...
Event(n).info = 'Recon for Doppler'; 
Event(n).tx = 0;         % no transmit
Event(n).rcv = 0;        % no rcv
Event(n).recon = 2;      % reconstruction
Event(n).process = 7;    % process
Event(n).seqControl = 0;
n = n+1;

Event(n).info = 'Treat RF'; 
Event(n).tx = 0;         % no transmit
Event(n).rcv = 0;        % no rcv
Event(n).recon = 0;      % reconstruction
...
UI(n_ui).Statement = '[result,hv] = setTpcProfileHighVoltage(20,1);';n_ui = n_ui + 1; 
...
mot_vect_z = evalin('base', 'mot_vect_z');
mot_ref_pos_z = evalin('base', 'mot_ref_pos_z');
evt_firing = evalin('base', '2+(n_ray+5)*Nframe+3');
