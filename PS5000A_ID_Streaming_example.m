...
[status.setSimpleTrigger] = invoke(triggerGroupObj, 'setSimpleTrigger', 0, 450, 2);
...
overviewBufferSize  = 50000; % Size of the buffer to collect data from buffer.
segmentIndex        = 0;
...
sample_interval = 8e-8
set(streamingGroupObj, 'streamingInterval', sample_interval);
...
% I want a window of 100 us, with most of the samples happening
% post-trigger, just enough pre-trigger samples to ensure the peak is
% properly formed.
sample_window = 1e-4;
total_samples = sample_window / sample_interval;
...
while(hasAutoStopOccurred == PicoConstants.FALSE && status.getStreamingLatestValuesStatus == PicoStatus.PICO_OK)
    ...
    if (newSamples > 0)
        ...
        if max(bufferChAmV) == 500
            contains_peak = contains_peak +1;
        end
...
fprintf('%i loops with %i peaks, triggered %i times, fraction %f \n', loops, contains_peak, triggers, (contains_peak/loops))
