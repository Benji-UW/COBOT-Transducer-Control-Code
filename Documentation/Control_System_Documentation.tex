...
\definecolor{mpl-orange}{HTML}{ff7f0e}
...
Then we plot the points of each of these planes individually. Since our data initially had three spatial dimensions and one scalar value dimension, slicing three 2D spatial planes out of it allows us to more easily plot the data in a way that is visually intuitive (process shown below in \autoref{fig:dataSlice1})
...
\begin{figure}[ht]
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node[block] (A) {\includegraphics[width=4cm,height=4cm]{figures/data_slice1_6.jpg}};
            \node[block,below=of A] (B) {\includegraphics[width=4cm,height=4cm]{figures/data_slice2_6.jpg}};
        ...
        \end{tikzpicture}
    \end{center}
    \caption{}\label{fig:dataSlice1}
\end{figure}

Then we repeat the process the produce a 1-dimensional slice of the fitness function as we move through one degree of freedom, through the optimal point.
...
\begin{figure}[ht]
    \begin{center}
        \begin{tikzpicture}[
            ...
            % Each frame is ~4.666 cm wide, middle one is centered on the middle
            
            \node[block, draw=white] (A) {\includegraphics[width=14cm,height=4cm]{figures/IsolatedTrace2.jpg}};
            
            \node[above left=1mm and 46mm of A.south] (blueOrange) {};
            \node[above =1mm of A.south] (greenOrange){};
            \node[above right=1mm and 46mm of A.south] (blueGreen){};

            \node[block, below=3cm of A] (B) {\includegraphics[width=14cm,height=3cm]{figures/1dSlice-1.png}};
            
            \node[below left=1mm and 46mm of B.north] (green){};
            \node[below =1mm of B.north] (blue){};
            \node[below right=1mm and 46mm of B.north] (orange){};

            \draw[mpl-blue, arrow]
            (blueOrange) edge[out=300,in=90,->] (blue);
            \draw[mpl-orange, arrow]
            (blueOrange) edge[out=315,in=100,->] (orange);
            \draw[mpl-green, arrow]
            (greenOrange) edge[out=240,in=80,->] (green);
            \draw[mpl-orange, arrow]
            (greenOrange) edge[out=300,in=100,->] (orange);
            \draw[mpl-blue, arrow]
            (blueGreen) edge[out=240,in=90,->] (blue);
            \draw[mpl-green, arrow]
            (blueGreen) edge[out=225,in=80,->] (green);

        \end{tikzpicture}
    \end{center}
    \caption{}\label{fig:dataSlice2}
\end{figure}
