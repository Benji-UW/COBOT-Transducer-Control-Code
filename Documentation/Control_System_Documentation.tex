...
\node[block, right = of sensor] (robot) {STANLEy};
\node[block, right = of robot] (URScript) {URScript};
\node[block, below = of sensor] (labview) {Signal Processing};
\node[block, right = of labview] (socket) {Internal Socket Server};
\node[block, right = of socket] (tcc) {Python Code};
